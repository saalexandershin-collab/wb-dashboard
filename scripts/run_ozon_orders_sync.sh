#!/bin/bash
# Синк заказов Ozon (FBO+FBS) + остатки → Supabase
# Крон: 0 3 * * *  (вместе с WB-заказами)
...
LOG_FILE="$PROJECT_DIR/logs/ozon_orders_sync.log"
...
for d in [prev, today]:
    print(f"  → Заказы Ozon {d.year}-{d.month:02d}...")
    env = {**env_base, "SYNC_YEAR": str(d.year), "SYNC_MONTH": str(d.month)}
    subprocess.run([sys.executable, "-u", f"{project_dir}/scripts/sync_ozon_orders.py"], env=env)

print("  → Остатки Ozon...")
subprocess.run([sys.executable, "-u", f"{project_dir}/scripts/sync_ozon_stocks.py"], env=env_base)
PYEOF

echo "$(date '+%Y-%m-%d %H:%M:%S') — Ozon заказы и остатки готово" >> "$LOG_FILE"
